...
public class EditFeatureTool implements OnGestureListener,UCFeatureLayerListener{
	...
	private int controlPointSize=60;
	
	private GeometryFactory gf=new GeometryFactory();
	
	public EditFeatureTool(UCMapView mapView,UCFeatureLayer layer)
	{
		mMapView=mapView;
		this.layer=layer;
	...
	public boolean onDown(MotionEvent e) {
		...
		for (int i=0;i<controlPoints.size();++i)
		{
			...
			if (Math.abs(screenPt.getX()-e.getX())<(controlPointSize/2) &&
				Math.abs(screenPt.getY()-e.getY())<(controlPointSize/2))
			{
				current=i;
				return true;
			}
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, float x, float y) {
		...
		if (current>=0)
		{
			...
			case GeometryType.Point:
				...
				feature=layer.updateFeature(feature, values);
				break;
			case GeometryType.LineString:
				...
				feature=layer.updateFeature(feature, values1);
				break;
			case GeometryType.Polygon:
				...
				feature=layer.updateFeature(feature, values11);
				break;
			}
			mMapView.refresh();
		}
